...
def initialize_db():
    with sqlite3.connect('bot.db') as conn:
        c = conn.cursor()
        c.execute('''CREATE TABLE IF NOT EXISTS excluded_channels (id INTEGER PRIMARY KEY)''')
        c.execute('''CREATE TABLE IF NOT EXISTS excluded_roles (id INTEGER PRIMARY KEY)''')
        c.execute('''CREATE TABLE IF NOT EXISTS user_special_points (user_id INTEGER PRIMARY KEY, points INTEGER)''')
        c.execute('''CREATE TABLE IF NOT EXISTS user_stars (user_id INTEGER PRIMARY KEY, star INTEGER)''')
        c.execute('''CREATE TABLE IF NOT EXISTS user_exp (user_id INTEGER PRIMARY KEY, exp INTEGER, level INTEGER DEFAULT 0)''')
...
def update_special_points(user_id, points):
    with sqlite3.connect('bot.db') as conn:
        c = conn.cursor()
        c.execute('INSERT OR REPLACE INTO user_special_points (user_id, points) VALUES (?, ?)', (user_id, points))
        conn.commit()

def get_special_points(user_id):
    with sqlite3.connect('bot.db') as conn:
        c = conn.cursor()
        c.execute('SELECT points FROM user_special_points WHERE user_id = ?', (user_id,))
        row = c.fetchone()
